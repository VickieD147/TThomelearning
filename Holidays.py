...
Target = [2]
if 'Ratings' > 'Target':
    print("The Destinations with Ratings below 2 are: ", data["Destination"], data["Rating"])
else:
    print("There are no destinations on file with a rating below 2")
data = pd.read_csv("Holidays.csv")
...
plt.show()
data = pd.read_csv("Holidays.csv")
plt.bar(data["Rating"], data["Destination"])
plt.title("Customer Ratings of Holiday Destinations")
